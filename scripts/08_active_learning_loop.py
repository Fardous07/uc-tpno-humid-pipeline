...
Fixes vs. original
------------------
1. BUG FIXED: @torch.no_grad() on score_candidates_with_model crashes at
   runtime.  The ICNN derives loadings via autograd.grad(omega, mu); inside
   torch.no_grad() requires_grad_(True) on mu is silently ignored, so
   autograd.grad raises RuntimeError.  Removed the decorator — model.eval()
   already gates dropout/batchnorm correctly.

2. BUG FIXED: physics_cfg in train_model_once() read from cfg root
   (cfg.get("physics_loss", {})) instead of from training section
   (train_cfg.get("physics_loss", {})).  Physics weights were always {}.

3. BUG FIXED: Wrong import path for GCMCRunner/GCMCConfig.
   parser.py is the RASPA output parser; the runner lives in runner.py.
...
# FIX: GCMCRunner/GCMCConfig live in runner.py, not parser.py
...
def water_saturation_pressure_antoine(temperature_k: float) -> float:
    """Antoine equation → saturation pressure of water in bar."""
    t_c   = temperature_k - 273.15
    a, b, c = 8.07131, 1730.63, 233.426
    p_mmHg = 10 ** (a - b / (c + t_c))
    return float(p_mmHg * 133.322368 / 1e5)
...
def relative_humidity_to_mole_fraction(
    ...
    p_sat = water_saturation_pressure_antoine(temperature_k)
    return min(rh * p_sat / total_pressure_bar, 0.9999)
...
def mixture_pressure_to_chemical_potentials(
    ...
    """Mole fractions + pressure → chemical potentials (kJ/mol)."""
    p_ref = 1.0  # bar
    mu: Dict[str, float] = {}
    for species, y_i in y.items():
        if y_i > 0:
            p_i = y_i * pressure
            mu[species] = R_KJ * temperature * float(np.log(p_i / p_ref))
        else:
            mu[species] = -100.0
    return mu
...
def build_flue_gas_composition(
    ...
    return {k: v / total for k, v in comp.items()} if total > 0.0 else None
        ...
        for P in pressures:
            for rh in rhs:
                ...
                mus = mixture_pressure_to_chemical_potentials(
                    y=comp, pressure=P, temperature=T
                )
                rows.append([
                    float(mus["CO2"]),
                    float(mus["N2"]),
...
def discover_candidate_cifs(
    ...
    return [
        ...
        if cif.stem not in trained and cif.stem in graph_mofs
    ]
...
def load_graph_for_mof(graph_dir: Path, mof_id: str):
    ...
    if not path.exists():
        raise FileNotFoundError(f"Graph file missing: {path}")
...
def _unpack_model_output(
    ...
    """
    Handle both dict and tuple returns from ThermodynamicPotentialNO.forward.

    Expected dict: {"q_pred": Tensor[B, P, C], "sigma": Tensor[B, P, C], ...}
    Expected tuple: (q_pred, log_var) or (q_pred, log_var, omega)
    """
    if isinstance(out, dict):
        q_pred = out["q_pred"].detach().cpu().numpy()
        sigma_t = out.get("sigma", None)
        sigma = sigma_t.detach().cpu().numpy() if sigma_t is not None else None
        return q_pred, sigma

    # tuple / list path
    if isinstance(out, (tuple, list)):
        q_pred = out[0].detach().cpu().numpy()
        if len(out) >= 2 and out[1] is not None:
            log_var = out[1].detach().cpu().numpy()
    ...
    raise TypeError(
        ...
        "Expected dict or tuple from ThermodynamicPotentialNO.forward."
    )


# FIX: @torch.no_grad() REMOVED.
# The ICNN derives loadings via autograd.grad(omega, mu).  Inside
# torch.no_grad(), requires_grad_(True) on mu is silently ignored and
# autograd.grad raises RuntimeError.  model.eval() is sufficient.
def score_candidates_with_model(
    ...
    """Score candidate MOFs on the fixed screening grid."""
    model.eval()

    cond_np, _ = build_screening_conditions()
    cond_base   = torch.tensor(cond_np, dtype=torch.float32, device=device)
    ...
    for start in range(0, len(candidate_cifs), max(candidate_batch_size, 1)):
        ...
        q_pred, sigma_arr = _unpack_model_output(out)

        if sigma_arr is None:
            sigma_arr = np.full_like(q_pred, 0.1, dtype=np.float32)

        for i, cif in enumerate(batch_cifs):
            mean_q_co2    = float(np.mean(q_pred[i, :, 0]))
            mean_sigma_co2 = float(np.mean(sigma_arr[i, :, 0]))
            max_sigma_co2  = float(np.max(sigma_arr[i, :, 0]))
...
def run_selected_gcmc(
    ...
    for cif in selected_cifs:
        for meta in condition_meta:
            ...
            logger.info(
                "GCMC [%d/%d] %s | T=%.2f K, P=%.4g bar, RH=%.2f%%",
                job_counter, total_jobs, cif.stem, T, P, rh * 100
            )
...
def results_to_adsorption_rows(results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in results:
        ...
        mus  = mixture_pressure_to_chemical_potentials(y=comp, pressure=P, temperature=T)
        rows.append({
            "mof_id":              r["mof_id"],
            "temperature":         T,
            "pressure":            P,
            "relative_humidity":   rh,
            "mu_CO2":              float(mus.get("CO2", -100.0)),
            "mu_N2":               float(mus.get("N2",  -100.0)),
            "mu_H2O":              float(mus.get("H2O", -100.0)),
            "T":                   T,
...
def prepare_indices(
    ...
    splitter = DataSplitter(
        method="random", test_size=0.1, val_size=0.1, random_state=seed
    )
...
def compute_normalization_stats(
    ...
    """
    Item-level normalization — excludes padded zeros (correct approach).
    Uses dataset[idx] directly, so no collate padding is involved.
    """
    ...
    for idx in train_indices:
        sample = dataset[idx]
        cond_list.append(sample["conditions"])  # [P, D]
        q_list.append(sample["loadings"])       # [P, C]

    if not cond_list:
        raise RuntimeError("Cannot compute normalization: training subset empty.")

    all_cond = torch.cat(cond_list, dim=0).float()  # [N_total, D]
    all_q    = torch.cat(q_list,    dim=0).float()  # [N_total, C]
...
def train_model_once(
    ...
    """Train or fine-tune the TPNO model for one AL iteration."""
    cfg        = load_yaml_config(config_path)
    model_cfg  = dict(cfg.get("model",    {}))
    train_cfg  = dict(cfg.get("training", {}))
    # FIX: physics_loss is nested under training, not at the top level
    ...
    train_idx, val_idx, test_idx, splits = prepare_indices(
        dataset=dataset, splits_file=splits_file, seed=seed
    )

    if len(train_idx) == 0:
        raise RuntimeError("Training split is empty during AL retraining.")
    if len(val_idx) == 0:
        raise RuntimeError("Validation split is empty during AL retraining.")

    train_loader = dataset.get_dataloader(
        indices=train_idx, batch_size=batch_size, shuffle=True,
        num_workers=num_workers, pin_memory=pin_memory,
    )
    val_loader = dataset.get_dataloader(
        indices=val_idx, batch_size=batch_size, shuffle=False,
        num_workers=num_workers, pin_memory=pin_memory,
    )
    ...
    if checkpoint_init is not None and Path(checkpoint_init).exists():
        logger.info("Warm-starting from checkpoint: %s", checkpoint_init)
    ...
    loss_cfg = LossConfig(
        ...
        henry_mu_threshold=float(
            physics_cfg.get("henry_mu_threshold",
    ...
    logger.info(
        "Retraining: train=%d val=%d test=%d epochs=%d",
        len(train_idx), len(val_idx), len(test_idx), epochs,
    )
    ...
    callbacks = CallbackRunner([
        MetricLogger(log_every=1, csv_path=str(output_dir / "metrics.csv")),
        TimingCallback(),
    ])

    def epoch_callback(epoch: int, metrics: Dict[str, Any]) -> None:
        physics_scheduler.step(epoch)
        callbacks.on_epoch_end(epoch, metrics, trainer)

    callbacks.on_train_begin(trainer)
    try:
        history = trainer.fit(n_epochs=epochs, callback=epoch_callback)
    finally:
...
def main() -> None:
    ...
    print(f"\nCurrent training MOFs: {len(trained_mofs)}")
    ...
    if args.raspa_data_dir:
        gcmc_config.raspa_data_dir = args.raspa_data_dir

    gcmc_runner = GCMCRunner(gcmc_config)
    ...
    for iteration in range(1, args.n_iterations + 1):
        ...
        for rank, mof_id in enumerate(selected_ids, start=1):
            row = score_df[score_df["mof_id"] == mof_id].iloc[0]
            print(
                f"  {rank:02d}. {mof_id} | "
        ...
        if new_rows:
            ...
            dedup_sub = ["mof_id", "mu_CO2", "mu_N2", "mu_H2O", "T"]
            ads_df    = ads_df.drop_duplicates(subset=dedup_sub, keep="last").reset_index(drop=True)
            n_rows_added = len(ads_df) - before
            print(f"Added {n_rows_added} new adsorption rows.")
            print(f"Total adsorption rows: {len(ads_df)}")
        else:
            print("No successful new adsorption rows produced.")
        ...
        if n_rows_added > 0:
            ...
            print("Skipping retraining — no new data added.")
        ...
        print(
            ...
            f"GCMC ok={success_count}/{len(sim_results)} | "
            f"rows added={n_rows_added} | "
            f"candidates remaining={len(candidate_cifs)}"
        )
    ...
    if failed_mofs_all:
        pd.DataFrame(failed_mofs_all).to_csv(
            out_dir / "failed_simulations.csv", index=False
        )
    ...
    print(f"  Final adsorption data: {out_dir / 'adsorption_final.parquet'}")
    print(f"  Final model          : {current_model_ckpt}")
    print()
    print("Next steps:")
    print(f"  python scripts/06_calibrate_uq.py  --model-checkpoint {current_model_ckpt}")
    print(f"  python scripts/09_benchmark.py      --model-checkpoint {current_model_ckpt}")
    print("=" * 80)
