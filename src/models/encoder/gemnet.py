...
class SphericalBasis(nn.Module):
    ...
    expansion of the angle theta between each edge triplet (i->j, j->k).
...
class GemNetBlock(nn.Module):
    ...
    def __init__(
        ...
        # Angular refinement (bilinear radial x angular -> hidden)
    ...
    def forward(
        ...
        triplet_index : ``[2, T]`` triplet indices.  Row 0 = edge (i->j)
                        index in ``edge_index``, row 1 = edge (j->k) idx.
...
def build_triplets(
    ...
    for j_node, cnt in zip(unique_dst.tolist(), counts.tolist()):
        in_idxs = sort_idx_dst[offset : offset + cnt]
        offset += cnt

        if j_node not in out_edges:
            continue
        out_idxs = out_edges[j_node]
    ...
    # PBC correction (in-place via PyTorch __isub__; modifies vec_ij and vec_jk)
