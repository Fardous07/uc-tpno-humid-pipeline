...
# Derived
R_kJ: Final[float] = R / 1000.0        # Gas constant in kJ mol⁻¹ K⁻¹
kB_eV: Final[float] = kB / e_charge    # Boltzmann constant in eV K⁻¹
...
# ═══════════════════════════════════════════════════════════════════════
#
# Each gas species has thermodynamic and molecular parameters required
# for EOS calculations, fugacity corrections, and chemical-potential
# conversions.  References are annotated inline.
# ═══════════════════════════════════════════════════════════════════════
...
class GasSpecies:
    ...
    def saturation_pressure(self, temperature: float) -> Optional[float]:
        ...
        if self.antoine_A is None:
            return None
        return 10.0 ** (self.antoine_A - self.antoine_B / (temperature + self.antoine_C))
...
# Convenient lookup dict keyed by formula string
...
# These can be used as scaffold labels for data splitting.
...
# (Boyd et al. 2019 Nature, Moghadam et al. 2017 Chem. Mater.)
...
class ForceField:
    ...
    fidelity_level: int = 1   # 1=low (generic), 2=mid (transferable), 3=high (polarizable)
...
# F3 = DFT+D, F4 = AIMD.  Costs are normalised so F1 = 1.
...
# ═══════════════════════════════════════════════════════════════════════
# Centralised defaults keep config YAML files short and auditable.
...
class EncoderDefaults:
    ...
    lmax: int = 2
    n_rbf: int = 32
    cutoff: float = 6.0          # Å — slightly larger than typical for MOFs
    use_pbc: bool = True
...
SOFTPLUS_BETA: Final[float] = 1.0 # Default β for softplus
...
# ═══════════════════════════════════════════════════════════════════════
# 14.  PUBLIC API  (for ``from src.utils.constants import *``)
# ═══════════════════════════════════════════════════════════════════════

__all__ = [
    # Fundamental constants
    "R", "R_kJ", "NA", "kB", "kB_eV", "h_planck", "c_light",
    "epsilon_0", "e_charge",
    # Unit conversions
    "BAR_TO_PA", "PA_TO_BAR", "ATM_TO_PA", "ATM_TO_BAR",
    "EV_TO_J", "J_TO_EV", "HARTREE_TO_EV", "KCAL_TO_KJ",
    "BOHR_TO_ANGSTROM", "ANGSTROM_TO_M",
    "MMOL_G_TO_MOL_KG", "CM3STP_G_TO_MOL_KG",
    # Gas species
    "GasSpecies", "CO2", "N2", "H2O", "GAS_REGISTRY", "gas_properties",
    # Flue-gas conditions
    "FlueGasCondition", "FLUE_GAS_DRY", "FLUE_GAS_HUMID_LOW",
    "FLUE_GAS_HUMID_HIGH", "FLUE_GAS_CONDITIONS",
    # Adsorption regimes
    ...
    # Numerical safety
    "EPS", "LOG_EPS", "MIN_SIGMA", "MAX_LOG_VAR",
    # Element properties
